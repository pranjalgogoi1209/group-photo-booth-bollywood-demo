  ...
  return (
    <div className="sp-face-list">
      ...
      <div className="sp-select-face" onClick={() => setIsSelect(!isSelect)}>
        <div className="sp-face">
          {Selected ? (
            <Image src={Selected} width={80} alt="techkilla" height={80} />
          ) : (
            <AiOutlinePlus />
          )}
        </div>

        <div className="text-dark fw-bold px-3">Select Face</div>
        {isSelect && (
            ...
            {data?.map((curItem, keys) => {
              return (
                <div
                  className="sp-face"
                  key={keys}
                  onClick={() => handleSelect(keys, curItem)}
                >
                  <Image
                    src={"data:image/png;base64," + curItem}
                    alt="techkilla"
                    width={80}
                    height={80}
                  />
